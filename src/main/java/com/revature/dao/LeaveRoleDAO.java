...
public class LeaveRoleDAO {
	...
	public List<LeaveRole> list() {
		...
		String sql = "SELECT rl.ID,rl.ROLE_ID,CASUAL_LEAVE,SICK_LEAVE,PAID_LEAVE,MATERNITY_LEAVE,PATERNITY_LEAVE,PRIVILEGED_LEAVE,rl.CREATED_DATE,rl.MODIFIED_DATE FROM ROLE_LEAVES rl,ROLE r WHERE rl.ROLE_ID=r.ID";

		System.out.println("");
		List<LeaveRole> roleList = jdbcTemplate.query(sql, (rs, rowNo) -> {
